...
Bureaucrat::Bureaucrat(std::string name, int grade) : _name(name) {
        ...
        if(grade < MIN_GRADE) throw GradeTooHighException();
        _grade = grade;
}
...
Bureaucrat &Bureaucrat::operator++() {
        if(_grade - 1 < MIN_GRADE) throw GradeTooHighException();
        _grade--;
        return *this;
}
...
void Bureaucrat::signForm(Form &f) {
        ...
        catch(std::exception &e) {std::cout << *this << " couldn't sign " << f.getName() << " because " << e.what() << std::endl;}
}
